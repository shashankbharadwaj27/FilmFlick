// ProfileSelfPage.jsx - Level-Based Ticket Coloring
...
function LoggedInUserProfile() {
  ...
  const { favourites, reviews, loading: userDataLoading } = useSelector((state) => state.userData);
  const { followers, following, loading: socialLoading } = useSelector((state) => state.social);
  ...
  useEffect(() => {
    if (user?.username) {
      // Dispatch all data fetches
      dispatch(fetchUserFavourites({ username: user.username }))
        .finally(() => setLoadingStates(prev => ({ ...prev, favourites: false })));
      ...
      dispatch(fetchFollowers({ username: user.username }))
        .finally(() => setLoadingStates(prev => ({ ...prev, followers: false })));
  ...
  const isStatsLoading = loadingStates.followers || loadingStates.following || loadingStates.reviews;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className={`rounded-xl p-8 mb-8 ${darkMode ? 'bg-[#1E1E1E]' : 'bg-white'}`}>
        <div className="relative">
          {/* Main Ticket - Always show if user exists */}
          <div className={`bg-gradient-to-r ${ticketTier.color} rounded-lg p-6 relative overflow-visible shadow-xl`}>
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 pl-2 pr-2">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                {user.bio && (
                  <p className="text-white/90 bio-text text-sm max-w-2xl leading-relaxed">
                    ~ {user.bio}
                  </p>
                )}
                {user.location && (
              ...
              {/* Edit Button */}
              <button 
                className="bg-white/20 hover:bg-white/30 text-white backdrop-blur-sm px-6 py-2.5 rounded-lg font-semibold transition-all border border-white/30 tracking-wide"
          ...
          {isStatsLoading ? (
            ...
            <div className={`mt-6 grid sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 gap-4 ${darkMode ? 'text-white' : 'text-gray-800'}`}>
              <div 
          ...
          {reviewCount < 100 && (
            <div className={`mt-4 p-3 rounded-lg ${darkMode ? 'bg-gray-700/50' : 'bg-gray-100'}`}>
        ...
        <section className={`rounded-lg p-5 ${
          ...
          {loadingStates.favourites ? (
            ...
            <div className={`text-center py-10 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              <p className="text-base mb-2 font-semibold">⭐ No favourites yet</p>
              <p className="text-sm">Add movies to your favourites to see them here!</p>
            </div>
          )}
        </section>
          ...
          {loadingStates.reviews ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
              {recentReviews.slice(0, 6).map((review) => (
                <Link 
                  to={`/${review.username}/review/${review.review_id}`} 
            ...
            <div className={`text-center py-10 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              ...
              <p className="text-sm">Start reviewing movies to build your collection!</p>
            </div>
          )}
        </section>
      </div>
